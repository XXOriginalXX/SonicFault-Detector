...
class MlService {
  ...
  // ── Main predict — tries backend first, falls back to offline RF ──────────
  Future<DetectionResult> predictFromBytes(Uint8List wavBytes, String fileName) async {
    ...
    // Fall back to offline RF using Dart MFCC
    if (_ready) {
  ...
  // ── Backend prediction via /upload-audio ─────────────────────────────────
  Future<DetectionResult?> _predictBackend(Uint8List bytes, String fileName) async {
    ...
    // If model not loaded on backend, fall through to offline
    if (key == 'unknown') return null;

    // Build allScores from classes we know
    final allScores = _classes.isNotEmpty
        ? _classes.map((c) => LabelScore(
        _fmt(c), c.toLowerCase() == key ? 1.0 : 0.0)).toList()
  ...
  // ── Offline predict — called from live screen with pre-extracted features ─
  DetectionResult predict(List<double> features) {
    if (!_ready) return _offlineResult();
...
class _Node {
  ...
  factory _Node.split(
      {required int feat, required double thresh,
...
class WavDecoder {
  ...
  static Float32List _resample(Float32List src, int srcSr, int dstSr) {
    final g = _gcd(srcSr,dstSr); final down = srcSr~/g; final up = dstSr~/g;
    if (up==1) {
      final out = Float32List((src.length+down-1)~/down);
      for (int i=0;i<out.length;i++) out[i]=src[i*down];
      return out;
    }
    final ratio=srcSr/dstSr; final out=Float32List((src.length/ratio).floor());
  ...
  static int _gcd(int a,int b){while(b!=0){final t=b;b=a%b;a=t;}return a;}
}

// ═════════════════════════════════════════════════════════════════════════════
//  MFCC extractor (used by live screen only — upload goes to backend)
// ═════════════════════════════════════════════════════════════════════════════
class MfccExtractor {
  static const int _nMfcc=40,_maxLen=300,_nMels=128,_nFft=2048,_hopLen=512,_sr=22050;
  static const int _nFreq=_nFft~/2+1;
  static Float32List? _hann; static Float32List? _fb; static List<Float32List>? _dct;

  static Float32List extract(Float32List audio) {
    _hann??=_makeHann(); _fb??=_makeMelFb(); _dct??=_makeDct();
    const pad=_nFft~/2;
    final padded=Float32List(audio.length+2*pad);
    for(int i=0;i<pad;i++){final s=pad-i;padded[i]=s<audio.length?audio[s]:0.0;}
    for(int i=0;i<audio.length;i++) padded[pad+i]=audio[i];
    for(int i=0;i<pad;i++){final s=audio.length-2-i;padded[pad+audio.length+i]=s>=0?audio[s]:0.0;}
    final result=Float32List(_nMfcc*_maxLen); int fi=0;
    for(int start=0;start+_nFft<=padded.length&&fi<_maxLen;start+=_hopLen){
      final power=Float32List(_nFreq);
      for(int k=0;k<_nFreq;k++){
        double re=0,im=0; final f=2*math.pi*k/_nFft;
        for(int n=0;n<_nFft;n++){final w=padded[start+n]*_hann![n];re+=w*math.cos(f*n);im-=w*math.sin(f*n);}
        power[k]=re*re+im*im;
      }
      final logMel=Float32List(_nMels);
      for(int m=0;m<_nMels;m++){double s=0;for(int k=0;k<_nFreq;k++) s+=_fb![m*_nFreq+k]*power[k];logMel[m]=math.log(s<1e-10?1e-10:s);}
      for(int c=0;c<_nMfcc;c++){double s=0;final row=_dct![c];for(int m=0;m<_nMels;m++) s+=logMel[m]*row[m];result[c*_maxLen+fi]=s.isFinite?s:0.0;}
      fi++;
    }
    return result;
  }

  static Float32List _makeHann(){final w=Float32List(_nFft);for(int i=0;i<_nFft;i++) w[i]=0.5*(1-math.cos(2*math.pi*i/_nFft));return w;}

  static Float32List _makeMelFb(){
    const fSp=200.0/3,minLogHz=1000.0,minLogMel=minLogHz/fSp;
    final logStep=math.ln2/6.0;
    double hzToMel(double hz)=>hz>=minLogHz?minLogMel+math.log(hz/minLogHz)/logStep:hz/fSp;
    double melToHz(double mel)=>mel>=minLogMel?minLogHz*math.exp(logStep*(mel-minLogMel)):fSp*mel;
    final melMin=hzToMel(0),melMax=hzToMel(_sr/2.0);
    final melPts=List<double>.generate(_nMels+2,(i)=>melMin+(melMax-melMin)*i/(_nMels+1));
  ...
  static List<Float32List> _makeDct()=>List.generate(_nMfcc,(c){
    ...
    return Float32List.fromList(List.generate(_nMels,(m)=>norm*2*math.cos(math.pi*c*(2*m+1)/(2*_nMels))));
  });
}